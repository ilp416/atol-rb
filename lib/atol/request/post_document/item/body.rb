      ...
      module Item
        class Body
          ...
          def setup_attributes(config, name, price, quantity, payment_method, payment_object, options)
            ...
            self.vat_type = options[:vat_type]
            self.vat_sum = options[:vat_sum]
          end
          ...
          def vat_info
            return if vat_type.nil? && vat_sum.nil?

            { vat: { type: vat_type.to_s, sum: vat_sum.to_f } }
          end
          ...
          def body
            ...
            }.merge(supplier_info.to_h, agent_info.to_h, vat_info.to_h)
          end
        end
      end
    end
